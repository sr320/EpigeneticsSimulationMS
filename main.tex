...
\begin{itemize}
    ...
    \item Writing text in the empirical sections. There are many locations labeled in all caps where CITATIONS or writing from NAMED INDIVIDUALS are needed!
...
-- frame marine inverts choice as a way to create a model that covers environmentally/ecologically relevant organisms and can be expanded to other organisms later
...
\textit{Text from Chema:} The emergence in the field of epigenetics has been critically supported by the immense increase in the availability of experimental ancomputational technologies for the study of genome structure and function. Yet, current challenges for the development of the field include the expansion of studies from model systems to environmentally and ecologically relevant organisms, the study of epigenetic mechanisms beyond DNA methylation (i.e., histone modifications and small RNAs), as well as their synergistic interactions modulating phenotypic responses. However, perhaps one of the most daunting tasks involves the integration of different types of information across biological levels, to make informed predictions and formulate hypotheses about the mechanistic basis underlying epigenetic responses and their implications for individuals and populations. The present work constitutes, to our knowledge, the first effort to provide such a mathematical framework combining environmental tracking by means of epigenetic modifications. Given the complexity of the mechanistic underpinnings at the interface between environment and gene function, this effort is focused on DNA methylation and the phenotypic and fitness consequences for organisms throughout time. In doing so, this model sets the foundations for future ….
...
How do organisms gather information about the environment around them, and is this information necessary for them to tune their epigenetic state to match environmental needs? While stress from environmental mismatch can be measured at the aggregate scale, it is less clear how organisms might identify which specific epigenetic changes should be made based on environmental cues. Therefore, we first used our model to investigate what information organisms need to detect about their (mis)match to the environment in order to track the environment epigenetically.
...
What empirical evidence exists that marine invertebrates use environmental information to directionally shift their epigenetic states in order to track the environment? Numerous studies have shown that organism phenotypes may vary predictably across gradients in environmental condition (CITATIONS) and may shift directionally in response to environmental perturbations like ABC (CITATIONS) and DEF (CITATIONS). 

In marine invertebrates, a growing body of evidence is beginning to link these phenotypic changes to epigenetic changes by examining how epigenetic state varies in response to acute environmental shifts and fluctuating environmental conditions. For example, oysters from the genus \textit{Crassostrea} respond to ocean acidification (OA) through epigenetic changes in mantle \citep{downey2020ocean,chandra2021oyster} and reproductive \citep{venkataraman2020general,venkataraman2022differential} tissue and in larvae \citep{dang2023epigenetic,lim2021dna}. In geoducks, changes to DNA methylation persist for months after exposure to OA, even after the environmental perturbation ends \citep{putnam2022dynamic}. In a few coral studies, changes in epigenetic state have been linked mechanistically to physiological responses to environmental change. For example, 
\citet{liew2018epigenome} demonstrated that exposure of \textit{Stylophora pistillata} to low pH resulted in changes in skeletal porosity underpinned by a \textbf{DIRECTION} in gene expression of \textbf{NUMBER OR IDENTITY} genes that showed differential DNA methylation.


Some long-term studies have also demonstrated seasonal variability in epigenetic state. For example, studies in the flat tree oyster \textit{Isognomon alatus} revealed a certain level of seasonality in temporal DNA methylation patterns, which seemed to be primarily associated with changes in temperature and horizontal visibility \citep{suarez2019seasonal}, and DNA methylation in the staghorn coral \textit{Acropora cervicornis} is correlated with seasonality (via changes in seawater temperature) \citep{rodriguez2020genome}.
...
%The empirical evidence for environmental tracking via epigenetic modification in marine invertebrates, is limited, however there are a handful of studies can be separated in two categories; acute assessments in epigenetic state upon environmental change and long-term assessments of epigenetic state in relation to dynamic environmental conditions. Much of the work looking at acute responses include work in molluscs and cnidarians. For molluscs, particular attention has been given to changes in response to ocean acidification. Shifts in DNA methylation  have be found in response to ocean acidification in the \textit{Crassostrea viginica} in mantle \citep{downey2020ocean} and gonad \citep{venkataraman2020general} the mantle \citep{chandra2021oyster} and larvae \citep{lim2021dna, dang2023epigenetic} of \textit{Crassostrea hongkongensis}, and the reproductive tissue \citep{venkataraman2022differential} of female \textit{Crassostrea gigas}. These shifts are as might be expected associated with different gene sets depending on the tissue. Variability can be seen across sex to as in a recent study where fter a 30-day exposure to control (572 ppm) or elevated pCO2 (2,827 ppm) conditions, whole genome bisulfite sequencing revealed 89 differentially methylated loci in females and 2,916 differentially methylated loci males (2,916) \cite{venkataraman2024dna}. In juvenile Pacific geoduck (\textit{Panopea generosa}) exposed to ocean acidification, DNA methylation changes were described to persist even after removal from the environmental perturbation \citep{putnam2022dynamic}. Specifically following four months of ambient common-garden conditions, juveniles initially exposed to low pH showed compensatory larged relative growth, with DNA methylation indicative of these phenotypic differences, demonstrating epigenetic carryover effects persisted months after initial exposure \citep{putnam2022dynamic}. 


%In cnidarians, the response to ocean acidification (OA) and DNA methylation has been less studied. In the initial landmark study of two coral species response to OA, the environmentally sensitive species, \textit{Pocillopora acuta} showed significant change in global DNA methylation associated with a change metabolomic and organismal phenotypes \cite{putnam2016ocean}. Combining organismal phenotype, DNA methylation, and gene expression, \cite{liew2018epigenome} demonstrated that exposure of \textit{Stylophora pistillata} to low pH for X months resulted in a change in skeletal porosity driven by a change in cell size underpinned by a X in gene expression of Y genes that also showed differential DNA methylation. 

TKTKTKTK? Further, in multi-faceted natural environmental conditions, changing light in \textit{Acropora palmata} (Gomez-Campo et al 2023) 

TKTKTKTK? changing from reef to mangrove in \textit{Porites porites} (Dimond et al 

%The study of temporal changes in genome-wide patterns of DNA methylation in the staghorn coral \textit{Acropora cervicornis} revealed a total of 83 loci displaying changes in DNA methylation that were significantly correlated to seasonal variation during a 17-month period, as determined mostly by changes in seawater temperature. Remarkably, the observed time-dependent variation in DNA methylation patterns is consistent across coral genets, coral source sites and site-specific conditions studied \citep{rodriguez2020genome}.

TKTKTKTK? but a recent study in the coral Acropora millepora found that the response to ocean acidification
...
Our model posits that these directional changes arise gradually, from the accumulation of a few directional (a.k.a. information-based) marker removals amidst a largely random set of additions and removals. In this way, our model mimics the effort of cells to harness chemical entropy to generate directed chemical forces \citep{baker2023cells}. Accordingly, an increasing intensity and duration of environmental signals will promote higher chemical entropy levels within the cell, which translates into increasing concentrations of substrates and/or cofactors available for the epigenetic regulation of genome function as a response. Thus, the epigenetic responses driving phenotypic responses will be better described as a continuous parameter, which will approach an maximum (optimal) response as a function of the intensity of the signal, prior exposure and genetic outfit \citep{hackerott2021coral}. 
...
Further, organisms' investments in epigenetic modification are likely also modulated by their environmental sensitivity. For example, the initial indication of inducible epigenetic change by DNA methylation in corals was documented in the environmentally sensitive P. acuta, but not the environmentally more tolerant M. capitata \cite{putnam2016ocean}.
